...
function LevelModel() {
    ...
    self.problemListClick = function (problem) {
		...
		self.addedCurrProblem = null;
		self.currProblem["id"]= self.currProblem.id;
		self.currProblem["title"]= self.currProblem.title;
		// (self.currProblem);
	};
...
function Level(data){  //full problem object
	...
	self.image = ko.observable((data && data.image)?data.image:"default.jpg");
    self.background = ko.observable(data?data.background:"");
	self.score = ko.observable(data?data.score:1);
