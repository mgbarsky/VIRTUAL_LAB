...
def db_get_student_courses(con, id):
    sql = "SELECT * FROM CourseStudent WHERE student_id = :student_id"
...
def db_insert_student(conn, student_obj):
    ...
    sql_student = "INSERT INTO Student " \
        "VALUES(:id, :email, :screen_name, :password, :first_name, :last_name, :image, :dob, :background)"
    ...
    sql_new_course = "INSERT INTO CourseStudent VALUES(:course_schedule_id, :student_id)"
    new_course_params = {"course_schedule_id": student_obj['course_schedule_id'], "student_id": student_obj['id']}
...
def db_get_game_bundles(con, id):
    sql = "SELECT * FROM CourseBundle WHERE course_schedule_id = :id"
    params = {'id': id}
    rows, error = db_query(con, sql, params)
    if rows is None:
        return None, error
...
def db_delete_game(conn, id):
    ...
    # delete tests
    b_sql = "DELETE FROM CourseBundle WHERE course_schedule_id = :course_id"
